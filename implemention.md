...
## Working mode
Implement **one phase at a time**, verifying the backend migrations AND the frontend wiring for that phase before moving to the next. Phases 0-4 (backend + frontend wiring) are done and verified, including the piece deferred from Phase 3. Next up is Phase 5 (trust score + match-timeout cron).
