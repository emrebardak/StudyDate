...
- On launch, calls `supabase.auth.getSession()` once (shown as a brief splash spinner) and sets `initialRouteName` to `MainTabs` if a session already exists, otherwise `RegisterVerification` — a returning user with a valid session skips registration entirely. **Session 24**: the same check now also gates on `email_verified` (checked before the existing `!row?.name` incomplete-registration check) — an authenticated-but-unverified session routes to `RegisterEmailCode`, not `MainTabs`
- **Session 26**: the `email_verified`/profile-completeness row query failing (network hiccup, RLS misconfig, or a `handle_new_user()` trigger-lag race right after signup) no longer falls open to `MainTabs` — that fail-open behavior was Session 22's, made for the unrelated `!row?.name` profile-completeness check, and doesn't belong on a security gate. A query error now shows a blocking "Couldn't confirm your account status" screen with a **Retry** button (`checkError` state, re-runs the identical `resolveSession()` check) instead of guessing; the `!row?.name` check's own fail-open behavior is unchanged, since a blank-profile inconvenience and a bypassed verification gate have genuinely different risk profiles
...
- Doubles as the login screen: a `mode` toggle (`'signup' | 'login'`) switches the same email/password fields between `supabase.auth.signUp` → **`RegisterEmailCode`** (was `RegisterProfile` directly before Session 24) and `supabase.auth.signInWithPassword`
- No separate LoginScreen file — this was a deliberate choice to reuse the existing form rather than duplicate it
- **Session 25**: the login branch now checks `email_verified` before deciding where to go — since `enable_confirmations = false` locally, `email_verified` was the app's *only* verification gate, and this path used to skip it entirely (any signed-up-but-uncoded account could switch to "Log In" and land straight in `MainTabs`). After a successful `signInWithPassword`, it fetches the caller's own `email_verified` (`users_select_own` RLS) and routes to `RegisterEmailCode` (passing `{ email: trimmed }`, same literal shape the signup path already uses) if false; otherwise `navigation.reset`s to `MainTabs` as before. Fails open on a transient fetch error, mirroring `AppNavigator.tsx`'s identical precedent. The equivalent `!row?.name` (incomplete-registration) gap on this same login path is still **not** closed — out of scope for this fix, which targeted `email_verified` only

**Registration Step 1.5 / Email Code** ([`RegisterEmailCodeScreen.tsx`](../StudyMatch/src/screens/RegisterEmailCodeScreen.tsx), new in Session 24)
- Offline/dev stand-in for real email verification (see backend-dev.md's Migration 20 and `EMAIL_VERIFICATION_MODE` in [`src/lib/config.ts`](../StudyMatch/src/lib/config.ts)): fetches and displays the caller's own `verification_code` (RLS-permitted own-row `SELECT`), clearly labeled as dev/offline, not a real "email sent" message
- 6-digit input → `supabase.rpc('verify_email_code', { p_code })`; only advances to `RegisterProfile` on success. A `ST013` (already-verified) response is treated as success, not an error — covers a resumed session or a double-tap race
- "Regenerate code" → `supabase.rpc('regenerate_verification_code')`, which returns `void` (writes the new code + a 15-minute expiry server-side); the screen re-fetches its own row afterward rather than trusting a client-visible RPC return value
- Error codes `ST012`–`ST015` mapped via `toFriendlyErrorMessage`'s `codeMessages`, same pattern as `DashboardScreen.tsx`'s `cancel_study_date` handling
