...
### Email Verification — Implemented, offline mode only (Session 24)

Separate from the `.edu` domain gate above: this proves the account holder actually controls the inbox at signup time, via a real random code instead of Phase 1's hardcoded `'000000'` mock. Implemented in `email_verification.sql` (Migration 20):

- `public.users.verification_code` (already existed) is now populated with a real random 6-digit value by `handle_new_user()` at signup, alongside a new `verification_code_expires_at` (15 minutes — asked directly rather than guessed) and `email_verified BOOLEAN DEFAULT false`.
- `verify_email_code(p_code)` and `regenerate_verification_code()` — both `SECURITY DEFINER`, operating only on `auth.uid()`'s own row — are the sole way to check or refresh the code. Distinct `ST012`–`ST015` error codes (`NOT_FOUND`/`ALREADY_VERIFIED`/`CODE_EXPIRED`/`INVALID_CODE`) let the client distinguish failure reasons, continuing this project's `ST0xx` convention rather than a new prefix.
- `protect_privileged_user_columns()` (Migration 8) was extended to guard the three new/changed columns — without this, a client could `PATCH /rest/v1/users {email_verified: true}` directly and self-verify without ever knowing the real code, the same class of bug Session 6 closed for `active_match_id`/`trust_score`.
- **Offline mode**: no email is ever sent. `RegisterEmailCodeScreen.tsx` reads `verification_code` directly off the user's own row (RLS-permitted own-row SELECT) and displays it with a dev/offline banner, standing in for "the email arrived." Both RPCs deliberately `RETURN void` rather than the code itself, specifically so a future *online* mode (real email delivery via an Edge Function/Database Webhook) can be added without changing either RPC's signature — only the frontend's display path would change.
- `AppNavigator.tsx` gates session routing on `email_verified`: an authenticated user who hasn't verified is routed to `RegisterEmailCode`, not `MainTabs`.

Verified via real REST calls against the local stack — see `docs/development.md` Session 24 for the full 17-check script (wrong/expired code rejection, correct-code success, already-verified rejection, regenerate producing a new code that invalidates the old one, and the direct-client-write block).
...
- **−10**: Last-minute cancellation — **implemented (Session 18)**, `last_minute_cancellation.sql`. A dedicated `cancel_study_date(study_date_id)` RPC, not a survey answer — the survey stayed binary. Attribution is `auth.uid()` at call time (never a parameter); "last-minute" is a named constant, `scheduled_time - NOW() < INTERVAL '2 hours'`; the −10 applies to the **caller** (safe here specifically because "did you cancel within the window" is an objective, unfakeable timestamp fact, unlike a peer-reported behavioral claim). A `BEFORE UPDATE` guard trigger (`protect_study_date_cancellation()`, deliberately `SECURITY INVOKER` — same Session 6 reasoning as `protect_privileged_user_columns()`) blocks any direct client write to `status='cancelled'`/`cancelled_by`/`cancelled_at`, forcing everything through the RPC. `submit_post_date_survey` was updated in the same migration to reject an already-cancelled study date, closing a double-penalty vector (cancel for −10, then also get surveyed `met=false` for −25 on the same date) that existed the moment a cancellation concept existed at all. See `docs/development.md` Session 18 for the 3 design options considered and why this one (of 3) was chosen.
...
- [x] Offline email verification (real random codes + expiry, not real email delivery) — Phase 1's mock `'000000'` default replaced by `email_verification.sql` (Migration 20, Session 24): `handle_new_user()` now seeds a real random 6-digit code + 15-minute expiry via a shared `generate_verification_code()` helper; `verify_email_code(p_code)`/`regenerate_verification_code()` are the only way to check/refresh it; `protect_privileged_user_columns` extended to block direct client writes to `email_verified`/`verification_code`/`verification_code_expires_at`. Verified via real REST calls against the local stack (wrong code → `ST015`, direct client self-verify → blocked `ST002`, correct code → `email_verified` flips true). **Real email delivery (an actual "online mode") is still not implemented** — see `EMAIL_VERIFICATION_MODE` in `src/lib/config.ts`, currently hardcoded `'offline'`
...
- [x] Matched-partner/swiped-right profile reads column-scoped — `matched_users_view.sql` + `swiped_right_users_view.sql` (Migrations 21/22, Session 25): `users_select_matched`/`users_select_swiped_right` (row-only RLS, no column restriction) both dropped and replaced with views reusing `discoverable_users`' vetted column list, closing a leak where a matched/swiped-right partner could read `verification_code`/`verification_code_expires_at` directly off `public.users`. `ChatScreen.tsx`/`MatchFoundScreen.tsx`/`ConversationsListScreen.tsx` → `matched_users`; `DashboardScreen.tsx`'s Recently Liked → `swiped_right_users`. See development.md Session 25
- [x] Phase 7 recommendation scoring, schema half — `recommendation_scoring.sql` (Migration 23, Session 26): nullable unbackfilled `users.birthdate DATE`; `compute_match_score()` (IMMUTABLE, named-constant weights: dept 30 / uni 15 / city 10 / shared tag 8 cap 5 / pacing 10 / audio 5 / fuel 5, `LEAST(100)`-clamped, explicitly NULL-safe — unknown contributes 0, never excludes); `discoverable_users` gains viewer-relative `age` (derived, raw birthdate never exposed) / `same_city` / `match_score` via `LEFT JOIN` on `auth.uid()`. Verified viewer-relative + clamp + NULL-partial-score at REST level.
- [x] Phase 7 recommendation scoring, frontend half — Session 30: `DiscoveryScreen.tsx` chains `.eq('university', ...)`/`.in('department', ...)`/`.gte`/`.lte('age', ...)`/`.eq('same_city', true)` (each only when actually set) + `.order('match_score', {ascending: false})`, plus a `"{score}% Match"` card badge; `FilterScreen.tsx` wired for real (mock distance slider replaced by a `sameCityOnly` toggle, defaults fixed so an untouched Apply is a true no-op); `EditProfileScreen.tsx` gained `city` + day/month/year `birthdate` inputs. Re-verified NULL-safety and each filter individually at REST level (8 synthetic accounts) — see development.md Session 30
